...
class LearningSwitch(api.Entity):
    ...
    def handle_link_down(self, port):
        for i in self.addressTable:
            if port==self.addressTable[i]:
                print("A link went down.")
    ...
    def handle_rx(self, packet, in_port):
        self.addressTable[packet.src]=in_port
        if packet.dst in self.addressTable:
            self.send(packet,port=self.addressTable[packet.dst])
        else:
