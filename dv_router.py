...
class DVRouter (basics.DVRouterBase):
  ...
  def handle_timer(self):
    for i in self.routeTable.keys():
      if(((api.current_time() - self.routeTable[i][2])<= self.ROUTE_TIMEOUT ) or (self.routeTable[i][2] == -1)):#Checks if a route is expired or not
        self.send(basics.RoutePacket(i,self.routeTable[i][0]),flood=True)#updates neighbours with distance vector table
  ...
  def handle_link_down (self, port):
    ...
    if self.POISON_MODE==True:
      ...
      for i in self.routeTable.keys():
        if self.routeTable[i][1] == port:
          del self.routeTable[i]#removes dictionary element from routeTable that has the value of the variable port
  ...
  def handle_rx (self, packet, port):
    ...
    if isinstance(packet, basics.RoutePacket):
      ...
      if (packet.dst in self.routeTable and  port != self.routeTable[packet.dst][1]):#checks if destination actually exists and that it isn't directly connected to the current router.
        if self.routeTable[packet.dst][0] <= INFINITY: #checks to ensure the latency is less than or equal to 16
          self.send(packet, self.routeTable[packet.dst][1])#sends packet out on the port corresponding to its intended destination
...
def HandleRoute(self,packet,port):
      ...
      if(total< INFINITY):
        if ((addr not in self.routeTable) or (self.routeTable[addr][0] >total)):
          pckt = basics.RoutePacket(addr,total)#makes packet with destination and cost to get to that destination
          self.routeTable[addr] = [pckt.latency,port,api.current_time()]#populate routeTable dictionary
          self.send(pckt, port, flood=True)#updates neighbours with the cost to get to the destination stored in the packet.
          if self.POISON_MODE==True:
          ...
          if self.routeTable[addr][1] == port:
            ...
            if total > self.routeTable[addr][0]:
              ...
              if self.POISON_MODE==True:
                ppckt = basics.RoutePacket(addr, INFINITY)#split horizon and poisoned reverse (advertising route to have a cost of infinity)
                self.send(ppckt, port, flood=False)
